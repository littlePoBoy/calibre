    ...
    def fetch_url(self, url):
        ...
        try:
            ...
            if hasattr(err, 'code') and err.code in responses:
                ...
                if err.code == 429:
                    print("retrying fetch link after 60s 11111111111111111 %s"% err)
                    time.sleep(60)
            ...
            if is_temp:  # Connection reset by peer or Name or service not known
                self.log.info('Temporary error, retrying in 1 second')
                time.sleep(1)
                with closing(open_func(url, timeout=self.timeout)) as f:
                    data = response(f.read()+f.read())
                    data.newurl = f.geturl()
        ...
        try:
            ...
            for c, tag in enumerate(tags):
                ...
                try:
                    ...
                    #print(' into Processing link function:self.fetch_url(iurl)'% iurl)
                    ...
                    self.log.debug(f'Processing images...{newbaseurl}')
                    ...
                    if recursion_level < self.max_recursions:
                        ...
                        self.log.info('Recursion limit reached. Skipping links in', iurl)
                    ...
                    print(f'process_links error {err}')
